...
- Tailwind CSS — identité « Nuit des Lumières » (depuis le 2026-07-07, remplace « terracotta & sapin » du 2026-07-05 ; validée par Natan : ton hybride pro/créateur, ancrage lyonnais) : encre bleu nuit `#141A2E` (héros/nav/footer via tokens `night-*`), action rouge lyonnais `#D22B24`, doré Fête des Lumières `#E8B23A` **réservé à l'argent et aux récompenses** (cagnottes, XP, badges), bleu profond `#24356B` pour les liens/éléments pro, fond blanc cassé chaud `#FBF8F3`. Polices : Inter (corps) + Archivo (titres, `font-display`), servies par `next/font`. Les tokens (modes clair + sombre, contrastes AA vérifiés par script à la refonte) vivent dans `src/app/globals.css` + `tailwind.config.ts` ; utiliser les classes composant de `globals.css` (`btn-primary`, `btn-accent`, `btn-outline`, `card`, `card-night`, `section-night`, `input`, `badge`, `badge-gold`, `chip`, `link`, `alert-error`) plutôt que des chaînes utilitaires ad hoc. Principe directeur de la refonte : zéro friction de compréhension — un écran compris en < 3 secondes, une seule action rouge par écran, placeholders exemples réalistes dans chaque champ de saisie
...
7. **Transfers adossés au paiement** (pre-mortem 2026-07-06) : `stripe.transfers.create` utilise `source_transaction` = le **Charge** (`ch_...`) associé à `challenges.stripe_payment_intent_id` (capté au webhook `checkout.session.completed`), récupéré via `stripe.paymentIntents.retrieve(...).latest_charge` juste avant l'appel — plutôt que de prélever sur la balance disponible globale de la plateforme — évite qu'un Transfer échoue faute de fonds disponibles quand plusieurs challenges se chevauchent sur un compte Stripe jeune. Correction du 2026-07-06 (E2E réel) : `source_transaction` n'accepte pas directement un PaymentIntent (`pi_...`), Stripe répond "No such charge" — un bug qui aurait fait échouer 100% des Transfers en prod, jamais détecté avant un vrai test avec de vraies clés
