...
- 2026-07-06 : Pre-mortem technique (même session/branche `review/bugfixes-and-design`) — durcissement du projet contre les causes de mort les plus probables identifiées, tout appliqué au code et aux docs. `migrations/2026-07-06-pre-mortem-hardening.sql` **confirmée appliquée en base** (MCP Supabase connecté et vérifié le 06/07 : colonnes `stripe_payment_intent_id`/`results_finalized_at`/`results_disputed_at` sur `challenges`, contrainte `payouts_status_check` avec `awaiting_review`, défaut `payouts.status` mis à jour).
  ...
  - **Hors de portée du code — actions de compte à faire par Natan avant tout lancement public** :
    - SMTP custom (Resend/Brevo) dans les settings Supabase Auth — le plan gratuit limite à quelques emails/heure, déjà bloquant pour le Bloc 19 et le sera pour de vraies inscriptions
    - Supabase Pro (backups quotidiens + PITR) dès qu'un vrai euro transite par la base — le plan gratuit n'a aucun backup automatique et se met en pause après 7 jours d'inactivité
    - Projet Supabase + clés Stripe séparés pour un environnement de staging, avant de réutiliser le projet actuel comme prod
    - Renseigner `RESEND_API_KEY`/`ADMIN_NOTIFICATION_EMAIL` et `CRON_SECRET` dans les variables d'env Vercel (`.env.local.example` à jour, valeurs réelles à générer/fournir)
